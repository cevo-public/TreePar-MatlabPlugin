...
% sampling at least one tip and on the STEM AGE of the tree.

% Function inputs:
% x - vector of grid points on which to evaluate the scale function W(x)
% (assumed to be an evenly spaced grid ranging from ZERO TO THE STEM AGE of 
% the tree)
...
function neglogLlhd = LikeliStem(x,params,cTimePts)
...
neglogLlhd = log(1-p+p*W(end)) - sum(log(p*Wderiv(cTimePts))) ...
